...
impl<'a, const SYNTAX_CHECK: bool> Parser<'a, SYNTAX_CHECK> {
    ...
    pub(crate) fn parse_call_expression(&mut self, callee: Expression) -> Expression {
        ...
        // In full parsing mode, check the AST node. In SYNTAX_CHECK mode,
        ...
        let is_eval_call = if let ExpressionKind::Identifier(ref id) = callee.inner {
            id.name == utf16!("eval")
        } else if SYNTAX_CHECK && callee.inner.classify() == ExpressionClass::Identifier {
    ...
    pub(crate) fn parse_template_literal(&mut self, is_tagged: bool) -> Expression {
        ...
        if needs_leading_empty && !SYNTAX_CHECK {
            if is_tagged {
        ...
        loop {
            ...
            if self.match_token(TokenType::TemplateLiteralString) {
                ...
                if !self.match_token(TokenType::TemplateLiteralString) && !SYNTAX_CHECK {
                    expressions.push(
                        self.expression(start, ExpressionKind::StringLiteral(Utf16String::new())),
                    );
                    if is_tagged {
