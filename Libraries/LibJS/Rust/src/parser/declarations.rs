...
impl<'a, const SYNTAX_ONLY: bool> Parser<'a, SYNTAX_ONLY> {
    ...
    /// Lazy-parse a function body.  When called from `Parser<false>` (normal
    /// mode), spawns a `Parser<true>` syntax checker.  When already inside a
    /// syntax checker (`Parser<true>`), uses brace-counting to skip — nested
    /// syntax checkers would have mismatched line numbers.
    pub(crate) fn parse_function_body_lazy(
        &mut self,
        is_async: bool,
        is_generator: bool,
    ) -> (FunctionBodyKind, bool, FunctionParsingInsights) {
        // In syntax-check mode, use brace-counting to skip nested function
        ...
        if SYNTAX_ONLY {
            ...
            return (FunctionBodyKind::Lazy(lazy), has_use_strict, insights);
        }

        // Normal mode: consume `{`, spawn a syntax-check parser.
        ...
        if self.class_scope_depth > 0 {
            if let Some(checker_names) = checker.referenced_private_names_stack.into_iter().next() {
                if let Some(outer_names) = self.referenced_private_names_stack.last_mut() {
