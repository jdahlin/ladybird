...
struct ScopeRecord {
    ...
    lazy_function_free_vars: HashSet<Utf16String>,
...
impl ScopeRecord {
    fn new(
        ...
        Self {
            ...
            lazy_function_free_vars: HashSet::default(),
...
impl ScopeCollector {
    ...
    pub fn mark_has_lazy_inner_function(&mut self) {
        ...
        let parent = match self.records[index].parent {
            Some(p) => p,
            None => return,
        };
        for si in ancestor_scopes(parent, &self.records).collect::<Vec<_>>() {
