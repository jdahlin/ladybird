...
/// Parse a lazily-deferred function body on demand.
///
/// Creates a temporary parser at `body_start_offset` (right after the
/// opening `{`), fully parses the body, runs scope analysis, and returns
/// the parsed AST, inner function table, and parsing insights.
fn parse_lazy_function_body(
    ...
    let mut parser = Parser::new_for_lazy_parse(
        ...
        0,
        0, // line/column not tracked for deferred parse error messages
        strict_mode,
        lazy.is_async,
        lazy.is_generator,
    );
        ...
        abort_on_panic(|| {
            ...
            if let ast::FunctionBodyKind::Lazy(ref lazy) = function_data.body {
                ...
                let (parsed_body, inner_table, insights) =
                    parse_lazy_function_body(lazy, &function_data, source_slice);
            ...
            let body_stmt = match &function_data.body {
                ast::FunctionBodyKind::Parsed(stmt) => stmt,
                ast::FunctionBodyKind::Lazy(_) => unreachable!("lazy body should have been parsed"),
            };
...
fn compute_sfd_metadata(function_data: &ast::FunctionData) -> SfdMetadata {
    let body_stmt = match &function_data.body {
        ast::FunctionBodyKind::Parsed(stmt) => stmt,
        ast::FunctionBodyKind::Lazy(_) => {
            unreachable!("lazy body should have been parsed before compute_sfd_metadata")
        }
    };
    let body_scope = match &body_stmt.inner {
